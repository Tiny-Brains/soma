-- Every statement the runner gate, the clocks and the season routes run against the schema,
-- PREPAREd so Postgres parses and plans each, then EXECUTEd by scenario.sql and the race files.
-- run.sh compares each shipped copy below with its workflow, so this harness walks what ships.
...
-- by hand once and went stale without anyone noticing, so run.sh asserts the copies are
-- identical rather than trusting that they are.
--
-- THE CLOCK STATEMENTS ARE COPIED VERBATIM TOO, from workflows/tb-*-run.json, and run.sh compares
-- them the same way (its PAIRS list names each). Regenerate them from the workflows; never retype one.
--
...
-- claim. $1 engine digest, $2 token, $3 lease seconds, $4 seats the caller can play,
...
-- start. $1 token, $2 runner, $3 match.
...
-- release -- not a fault and not a lapse. $1 token, $2 the refusal flag, $3 ceiling (the
-- fallback: a season's execution.refusal_ceiling wins), $4 runner, $5 match, $6 the grace in
-- seconds a row must have waited since it was paired before its refusals can fail it.
-- workflows/soma-runner-release.json / release
PREPARE k_release AS
WITH c AS (SELECT m.id, m.refusals + 1 >= coalesce(CASE WHEN (se.rules -> 'execution' ->> 'enabled')::boolean THEN (se.rules -> 'execution' ->> 'refusal_ceiling')::int END, ($3)::int) AND m.created_at <= now() - ($6)::int * interval '1 second' AS spent FROM matches m JOIN seasons se ON se.id = m.season_id WHERE m.id = ($5)::uuid) UPDATE matches SET status = CASE WHEN c.spent THEN 'failed' ELSE 'pending' END::match_status, refusals = refusals + 1, claim_token = NULL, lease_expires_at = NULL, fault_reason = CASE WHEN c.spent THEN 'MODEL_UNAVAILABLE' END, closed_at = CASE WHEN c.spent THEN now() END FROM c WHERE matches.id = c.id AND matches.claim_token = ($1)::uuid AND matches.status = 'claimed' AND ($2)::boolean AND matches.played_by = ($4)::uuid AND EXISTS (SELECT 1 FROM live_runners lr WHERE lr.id = ($4)::uuid);

-- renew, on the DATABASE's clock. $1 token, $2 lease seconds, $3 runner, $4 match.
...
-- finish -- the row and all its seats, or neither. $1 token, $2 match, $3 the result,
...
-- the trial insert's read, with the seat count coming from the board:
...
-- WHAT A COMPETITOR IS TOLD, copied verbatim from the generated clock
...
-- Season maps: the upload's insert and the enable/disable flip, from the routes that ship them.
PREPARE m_insert AS
