...
PREPARE k_read AS
WITH w AS ( SELECT m.id, m.seed, m.preset, m.seat_count, m.trial_version_id, m.strike_ceiling, (row_number() OVER (ORDER BY m.id) - 1)::int AS m FROM matches m WHERE m.claim_token = ($1)::uuid AND m.status = 'running' ) SELECT json_build_object( 'm', w.m, 'id', w.id, 'seed', w.seed, 'preset', w.preset, 'seat_count', w.seat_count, 'trial_model_id', w.trial_version_id, 'strike_ceiling', w.strike_ceiling, 'seats', (SELECT json_agg(json_build_object( 'm', w.m, 'seat', s.seat, 'model_id', s.version_id, 'strike_ceiling', w.strike_ceiling, 'weights_hash', s.weights_hash, 'adapter_hash', s.adapter_hash) ORDER BY s.seat) FROM match_seats s WHERE s.match_id = w.id) ) AS row FROM w ORDER BY w.m;
...
PREPARE k_finish AS
WITH m AS ( UPDATE matches SET status = 'finished', reason = ($4)::text, turns = ($5)::int, played_ms = GREATEST(0, (EXTRACT(EPOCH FROM (now() - ($6)::timestamptz)) * 1000)::int), engine_digest_played = ($7)::text, evaluator_digest = ($8)::text, replay_key = ($9)::text, played_at = now(), lease_expires_at = NULL WHERE id = ($2)::uuid AND claim_token = ($1)::uuid AND status = 'running' AND (SELECT count(DISTINCT v.seat) FROM jsonb_to_recordset(($3)::jsonb) AS v (seat smallint) WHERE v.seat BETWEEN 0 AND seat_count - 1) = seat_count RETURNING id ) UPDATE match_seats s SET rank = v.rank, score = v.score, strikes = v.strikes, infer_us_total = v.infer_us_total, infer_us_max = v.infer_us_max, infer_turns = v.infer_turns FROM m, jsonb_to_recordset(($3)::jsonb) AS v (seat smallint, rank smallint, score int, strikes smallint, infer_us_total bigint, infer_us_max int, infer_turns int) WHERE s.match_id = m.id AND s.seat = v.seat;
...
WITH live AS (
    ...
    SELECT md.id AS model_id, md.weight_class, u.role,
      ...
      JOIN models mde ON mde.id = md.model_id
      JOIN users u ON u.id = mde.owner_id
