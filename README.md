...
401 sends the caller to sign in, 403 refuses. Keep the 401/403 split. `tb-probe` also answers on
`POST /internal/probe/adapter`, unauthenticated, for the admit walk's `channel_call`; the port also
...
`ORION_ADMIN_API_KEY` (load-package). `scripts/measure/` holds the claim-cost
and autoscaler harnesses.
...
- **TLS**: `SOMA_COOKIE_SECURE=1`, and an https `OAUTH_REDIRECT_URI` (Orion refuses http off
  loopback).
- **Never expose port 8080 beyond the proxy.** It carries the admin API, `/metrics` and the
  unauthenticated `/internal/probe/adapter`. `admin_auth` is on, and `/health` detail needs the key.
...
- **Scale runners on demand, never on queue depth**: pair caps the queue at `pair_depth_target`.
  The demand view plus the scaling arithmetic is `scripts/measure/autoscaler.sql`.
...
scripts/gen-clocks.py       the clocks' readable SQL and task graphs
...
scripts/measure/            claim cost and autoscaler harnesses
```
...
- `[models] max_artifact_bytes` is 64 MiB while the default `large` class allows 128 MiB, so a large
  model between the two is refused `SIZE_FAILED`.
...
- `scripts/measure/autoscale.sh`, `autoscaler.sql` and `claim-load.sh` still name `matches.preset`
  and fail against the current schema.
